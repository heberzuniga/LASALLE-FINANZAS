...
# Crecimiento mensual del escenario base (25% por defecto)
growth_base = 25.0
for i in range(1, len(df_base)):
    ...
    df_base.loc[i, "Utilidad Operativa [Bs]"] = df_base.loc[i, "Ingresos [Bs]"] - df_base.loc[i, "COGS [Bs]"] - fixed_costs
...
df_resumen["Año"] = 1
df_annual = df_resumen.groupby("Año").agg({
    "Ingresos [Bs]": "sum",
    "COGS [Bs]": "sum",
    "Utilidad Operativa [Bs]": "sum"
}).reset_index()
...
if lang == "Español":
    ...
    - El modelo *Benessere Açaí Lite* proyecta un crecimiento mensual promedio del **{promedio_crecimiento:.1f}%**,
      alcanzando una utilidad acumulada de **Bs {total_utilidad:,.2f}** en 12 meses.
    ...
    st.success(f"""
    ### 🧠 Executive Summary
    - The *Benessere Açaí Lite* model projects an average monthly growth of **{promedio_crecimiento:.1f}%**,
      reaching an accumulated profit of **Bs {total_utilidad:,.2f}** over 12 months.
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, "Benessere Açaí Lite – Executive Financial Report", ln=True, align="C")
        self.ln(5)
